...
//           <Link
//             to={`/travel_itinerary/${index}/${encodeURIComponent(trip?.title)}`}
//           >
...
const ImmersiveCard = ({ trip }) => (
  ...
  >
    ...
    <div className="absolute inset-0 p-6 flex flex-col justify-end text-white">
      <h3 className="text-3xl font-bold mb-4">{trip.title}</h3>
      <div className="grid grid-cols-2 gap-4 mb-4">
        <div className="flex items-center gap-2"><Sun className="w-5 h-5 text-yellow-400" />{trip.duration}</div>
        <div className="flex items-center gap-2"><Mountain className="w-5 h-5 text-yellow-400" />{trip.difficulty}</div>
        <div className="flex items-center gap-2"><MapPin className="w-5 h-5 text-yellow-400" />{trip.distance}</div>
        <div className="flex items-center gap-2"><Waves className="w-5 h-5 text-yellow-400" />{trip.highlights?.length} Locations</div>
      </div>
    </div>
  </motion.div>
);
...
const TripCards = () => {
  ...
  useEffect(() => {
    axios.get("https://travel-backend-0cb0.onrender.com/api/trips") 
